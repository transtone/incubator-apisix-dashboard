...
<template>
  <div class="plugin-dialog">
    <el-dialog :title="'Plugin ' + name + ' Edit'" :visible.sync="showDialog">
      <el-form
        v-if="schema.oneOf"
        ref="form"
        :model="data"
        ...
        >
          ...
          >
            ...
            >
              ...
              />
              <el-button @click="deleteObjectItem(key, objectIndex)">{{
                $t("button.delete")
              }}</el-button>
            </div>
            <el-button @click="addObjectItem(key)">{{
              $t("button.addValue")
            }}</el-button>
          </div>
        </el-form-item>
...
export default class extends Vue {
  ...
  private addArrayItem(key: any) {
    if (
      this.arrayPropertiesLength[key].length <
      this.schema.properties[key].maxItems
    ) {
      this.arrayPropertiesLength[key].push(
        this.arrayPropertiesLength[key].length
      );
      this.$forceUpdate();
    } else {
  ...
  get oneOfPropHasEmptyValue() {
    return this.data.values
      ? this.data.values.find((value: string) => value === "")
      : true;
  }
  ...
  private removeOneOfPropValue(index: number) {
    this.data.values = this.data.values.filter(
      (item: any, _index: number) => index !== _index
    );
  }
